...
# Timeline plot presidentes       
         
str(presidentes)

presidentes$dias_desde_ind_inicio
...
plot<-ggplot()+
  #creado periodos de conflicto y bonanza
  annotate('rect',xmin=0,xmax=max(presidentes$orden_plot)+2,ymin=5270,ymax=6385,fill='#CC2C00',alpha=.7)+
  annotate('rect',xmin=0,xmax=max(presidentes$orden_plot)+2,ymin=8667,ymax=16228,fill='#345995',alpha=.7)+
  annotate('rect',xmin=0,xmax=max(presidentes$orden_plot)+2,ymin=15863 ,ymax=16593 ,fill='#CC2C00',alpha=.7)+
  annotate('rect',xmin=0,xmax=max(presidentes$orden_plot)+2,ymin=21070,ymax=22896,fill='#CC2C00',alpha=.7)+
  annotate('rect',xmin=0,xmax=max(presidentes$orden_plot)+2,ymin=43805,ymax=44013,fill='#CC2C00',alpha=.7)+
  annotate('rect',xmin=0,xmax=max(presidentes$orden_plot)+2,ymin=58001,ymax=64985,fill='#CC2C00',alpha=.7)+
  annotate('rect',xmin=0,xmax=max(presidentes$orden_plot)+2,ymin=65170,ymax=70284,fill='#345995',alpha=.7)+
  ...
  geom_text(aes(x=40,y=51489),label='Legu�a fue el presidente que\n gobernó más tiempo.\n15 años en total',fontface='bold',family='Thabit-Bold')+
  ...
  geom_text(aes(x=32,y=61900),label='Unico presidente asesinado\n en funciones',fontface='bold',family='Thabit-Bold')+
  geom_curve(aes(y=51818,yend=61900,x=27,xend=30.5),angle=125,curvature = 0.3,linetype=2)+
  #mandatos de cada presidente
  geom_rect(aes(ymin=dias_desde_ind_inicio,ymax=dias_desde_ind_fin,xmin=orden_plot-.45,xmax=orden_plot+.45,fill=Origen_del_cargo),size=.25,colour='black',data=presidentes)+
  geom_text(aes(x=orden_plot,y=dias_desde_ind_fin,label=Presidente),hjust=-.05,size=3,vjust=.4,data=presidentes,family='Thabit-Bold')+
  #formato
...
# anios de gobierno segun ocupacion
